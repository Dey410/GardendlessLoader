...
void main() {
  ...
  test('serves GET and HEAD with no-store and expected MIME', () async {
    ...
    final getRequest =
        await client.getUrl(Uri.parse('$localOrigin/index.html'));
    final getResponse = await getRequest.close();
    await getResponse.drain<void>();

    final headRequest =
        await client.headUrl(Uri.parse('$localOrigin/src/settings.json'));
    ...
    expect(
        getResponse.headers.value(HttpHeaders.cacheControlHeader), 'no-store');
    expect(headResponse.statusCode, HttpStatus.ok);
    expect(headResponse.headers.contentType?.mimeType, 'application/json');
  });

  test('rejects unsupported methods, missing files, directories, and traversal',
      () async {
    await server.start(root: temp);
    final client = HttpClient();

    final post =
        await (await client.postUrl(Uri.parse('$localOrigin/index.html')))
            .close();
    await post.drain<void>();
    final missing =
        await (await client.getUrl(Uri.parse('$localOrigin/nope.js'))).close();
    await missing.drain<void>();
    final directory =
        await (await client.getUrl(Uri.parse('$localOrigin/assets/'))).close();
    await directory.drain<void>();
    final traversal =
        await (await client.getUrl(Uri.parse('$localOrigin/../secret.txt')))
            .close();
  ...
  test('selfCheck verifies real server paths and wasm MIME', () async {
    await File(p.join(temp.path, 'assets', 'game.wasm'))
        .writeAsBytes([0, 97, 115, 109]);
...
Future<void> _writeFiles(Directory root) async {
  ...
  await File(p.join(root.path, 'index.html'))
      .writeAsString('<title>PvZ2 Gardendless</title>pvzge');
