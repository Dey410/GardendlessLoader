...
''');
  ...
  test('recovers the newest complete manifest after an interrupted commit',
      () async {
    final temp = await Directory.systemTemp.createTemp('gl_manifest_');
    addTearDown(() => temp.delete(recursive: true));
    final manifestFile = File(p.join(temp.path, 'manifest.json'));
    ...
    await File('${manifestFile.path}.tmp').writeAsString(
      '${jsonEncode(ResourceManifest.initial().copyWith(
            generation: 2,
  ...
  test('ignores a corrupt temporary manifest and keeps the complete main file',
      () async {
    final temp = await Directory.systemTemp.createTemp('gl_manifest_');
    addTearDown(() => temp.delete(recursive: true));
    final manifestFile = File(p.join(temp.path, 'manifest.json'));
    ...
    await File('${manifestFile.path}.tmp')
