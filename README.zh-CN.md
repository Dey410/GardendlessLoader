...
GP-Next ZIP 补丁包既可以把 `pack.json` 直接放在压缩包根目录，也可以统一放在唯一的一层外壳目录中。对于后一种结构，Loader 会在内部副本中自动去掉这一层，并忽略 `.DS_Store` 与 `__MACOSX` 元数据；用户选择的原始文件不会被修改。存在多个候选目录、外壳之外还有无关文件或嵌套超过一层时会拒绝导入。
