...
| 9 | iOS 音频改为原生优先：非短音效（超限/不支持容器/引擎不可用）与解码失败一律静音，不再回退或转交 WebKit；只有 JS 代理未拦截的 BGM/非 mp3-m4a 文件继续由 WebKit 自然播放；伪装 `.mp3` 的 M4A 统一按 `.m4a` 别名解码 | 用户可见音频行为改变 | 超限长音效将被静音 |
