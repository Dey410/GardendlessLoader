...
final class AppLogStore {
  ...
  static let channelName = "io.github.dey410.gardendlessloader/app_logger"
  ...
  private(set) var appSessionId = "unavailable"
  ...
  func install(messenger: FlutterBinaryMessenger) {
    ...
    channel.setMethodCallHandler { [weak self] call, result in
      ...
      switch call.method {
      case "initialize":
        result(["appSessionId": self.appSessionId])
      case "emit":
        guard let event = call.arguments as? [String: Any] else {
          result(FlutterError(code: "invalid_log_event", message: "Log event must be a map", details: nil))
          return
        }
        self.emit(event)
        result(nil)
      case "snapshot":
        ...
        result(self.snapshot(limit: limit))
      case "flush":
        _ = self.flush(timeout: 0.5)
        result(nil)
      case "deleteHistory":
        self.deleteHistory()
        result(nil)
  ...
  func initialize() {
    ...
    logsDirectory = support.appendingPathComponent("GardendlessLoader/logs", isDirectory: true)
    ...
    do {
      ...
      try JSONSerialization.data(withJSONObject: ["appSessionId": appSessionId])
  ...
  func emit(
    ...
    if let gameSessionId { value["gameSessionId"] = gameSessionId }
    if let operationId { value["operationId"] = operationId }
    if let context { value["context"] = context }
    if let error {
      value["error"] = [
  ...
  func endSession() {
    emit(source: "ios", level: "INFO", category: "app.lifecycle", event: "app_session_ended", outcome: "succeeded")
    _ = flush(timeout: 0.5)
    queue.sync {
      try? handle?.close()
      handle = nil
  ...
  func snapshot(limit: Int) -> [String: Any] {
    queue.sync {
      let files = (try? FileManager.default.contentsOfDirectory(
        at: logsDirectory, includingPropertiesForKeys: [.fileSizeKey]
      )) ?? []
      let total = files.reduce(0) { value, url in
        value + ((try? url.resourceValues(forKeys: [.fileSizeKey]).fileSize) ?? 0)
      }
      return [
        ...
        "events": Array(recent.suffix(max(1, min(500, limit)))),
      ]
    }
  }
  ...
  func deleteHistory() {
    ...
    queue.async { [weak self] in
      ...
      let files = (try? FileManager.default.contentsOfDirectory(at: self.logsDirectory, includingPropertiesForKeys: nil)) ?? []
      for file in files where file.pathExtension == "jsonl" && !file.lastPathComponent.hasPrefix(currentPrefix) {
  ...
  private func emitImmediate(_ input: [String: Any]) {
    ...
    var value: [String: Any] = [
      "schemaVersion": 1,
      "timestampUtc": ISO8601DateFormatter.gdl.string(from: Date()),
      "monotonicMs": Int((ProcessInfo.processInfo.systemUptime - startedAt) * 1000),
      "sequence": sequence,
      "level": Self.level(input["level"] as? String),
      "source": Self.identifier(input["source"] as? String, fallback: "ios"),
      "category": Self.identifier(input["category"] as? String, fallback: "unknown"),
      "event": Self.identifier(input["event"] as? String, fallback: "unknown_event"),
      "outcome": Self.identifier(input["outcome"] as? String, fallback: "observed"),
      "appSessionId": appSessionId,
    ]
    for key in ["code", "message", "gameSessionId", "operationId"] {
      if let text = input[key] as? String { value[key] = Self.sanitize(text, limit: key == "message" ? 2048 : 256) }
    }
    if let duration = input["durationMs"] as? NSNumber { value["durationMs"] = duration }
    if let context = input["context"] as? [String: Any] { value["context"] = Self.sanitizeMap(context) }
    if let error = input["error"] as? [String: Any] { value["error"] = Self.sanitizeMap(error) }
    ...
    do {
      ...
      if value["level"] as? String == "ERROR" || value["level"] as? String == "FATAL" {
  ...
  private func ensureHandle(nextBytes: Int) throws {
    let size = currentFile.flatMap { try? $0.resourceValues(forKeys: [.fileSizeKey]).fileSize } ?? 0
    if handle == nil || size + nextBytes > 2 * 1024 * 1024 {
      try? handle?.close()
  ...
  private static func sanitizeMap(_ value: [String: Any]) -> [String: Any] {
    let forbidden = try! NSRegularExpression(pattern: "password|passwd|secret|token|authorization|cookie|api.?key|private.?key", options: .caseInsensitive)
    var result: [String: Any] = [:]
    for (key, item) in value.prefix(32) {
      if forbidden.firstMatch(in: key, range: NSRange(key.startIndex..., in: key)) != nil { continue }
      if let text = item as? String { result[key] = sanitize(text, limit: 4096) }
      else if item is NSNumber { result[key] = item }
    }
    return result
  }
  ...
  private static func sanitize(_ value: String, limit: Int) -> String {
    ...
    result = result.replacingOccurrences(of: "/(Users|home)/[^/\\s]+", with: "/<user-home>", options: .regularExpression)
    return result.count > limit ? String(result.prefix(limit)) + "...[truncated]" : result
  }

  private static func identifier(_ value: String?, fallback: String) -> String {
    guard let value, value.range(of: "^[A-Za-z0-9._-]{1,128}$", options: .regularExpression) != nil else { return fallback }
    return value
  }

  private static func level(_ value: String?) -> String {
    let result = value?.uppercased() ?? "INFO"
    return ["DEBUG", "INFO", "WARN", "ERROR", "FATAL"].contains(result) ? result : "INFO"
  }
...
private extension ISO8601DateFormatter {
  static let gdl: ISO8601DateFormatter = {
